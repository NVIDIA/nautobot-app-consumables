...
def build(context: Context, force_rm: bool = False, cache: bool = True) -> None:
    ...
    print(f"Building Nautobot with python {context.nautobot_consumables.python_ver}")
...
def dbshell(context: Context, query: str = "", input: str = "", output: str = "") -> None:
    ...
    if "docker-compose.mysql.yml" in context.nautobot_consumables.compose_files:
    ...
    elif "docker-compose.postgres.yml" in context.nautobot_consumables.compose_files:
        command.extend([
            "psql",
...
def makemigrations(context: Context, name: str | None = None) -> None:
    ...
    command = ["nautobot-server", "makemigrations", context.nautobot_consumables.project_source]
