...
def get_docker_nautobot_version(context: Context) -> str:
    ...
    nautobot_ver = context.nautobot_consumables.nautobot_ver
    python_ver = context.nautobot_consumables.python_ver

    dockerfile_path = os.path.join(context.nautobot_consumables.compose_dir, "Dockerfile")
...
def docker_compose(context: Context, command: str, **kwargs: Any) -> Result:
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.nautobot_consumables.compose_http_timeout,
        "NAUTOBOT_VER": context.nautobot_consumables.nautobot_ver,
        "PYTHON_VER": context.nautobot_consumables.python_ver,
    }
    ...
    compose_dir = Path(context.nautobot_consumables.compose_dir)
    for compose_file in context.nautobot_consumables.compose_files:
...
def run_command(context: Context, command: str, service: str = "nautobot",
    ...
    if is_truthy(context.nautobot_consumables.local):
